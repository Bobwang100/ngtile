...
def convLayer(x, kHeight, kWidth, strideX, strideY,
              featureNum, name, padding="SAME", no_activation=False):
    """convlutional"""
...
class VGG19(object):
    ...
    def buildCNN(self):
        ...
        fc6 = convLayer(pool5, 7, 7, 1, 1, 4096, 'fc6', padding='VALID')
        dropout1 = dropout(fc6, self.KEEPPRO)
        fc7 = convLayer(dropout1, 1, 1, 1, 1, 4096, 'fc7')
        dropout2 = dropout(fc7, self.KEEPPRO)
        fc8 = convLayer(dropout2, 1, 1, 1, 1, self.CLASSNUM, 'fc8', no_activation=True)
        self.out = tf.reshape(fc8, [-1, self.CLASSNUM])
        ...
        # dropout2 = dropout(fc7, self.KEEPPRO)
        #
        # self.out = fcLayer(dropout2, 4096, self.CLASSNUM, True, "fc8")
